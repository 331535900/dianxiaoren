...
<div class="hy-box" style="width: 100%;">
    <div class="thread">
        
        <a href="<?php HYBBS_URL('thread',$v['tid']); ?>" class="title1"  ajax="true" pos="right"><?php echo $v['content'];?></a>
        <ul class="hy-lable-group">
            <li style="word-spacing: -0.35em;">
                <a href="" class="hy-lable hy-lable-success"><?php echo forum($forum,$v['fid'],'name'); ?></a>
            </li>
            <li>
                <a href="<?php HYBBS_URL('thread',$v['tid']); ?>" class="hy-lable hy-lable-zz" ajax="true" pos="right"><?php echo $v['title'];?></a>
            </li>
            <li>
                <span class="icon-reply"></span> <?php echo $_LANG['发表于'];?> <?php echo humandate($v['atime']); ?>
            </li>
        </ul>
...
<div class="hy-box" style="padding:10px">
    ...
    <span class="pg-item" disabled>
        <a class="hy-btn hy-btn-danger" href="<?php HYBBS_URL('my',$data['user'],['post'=>$pageid-1]); ?>"><?php echo $_LANG['上一页'];?></a>
    </span>
    <?php endif ?>
    ...
    <span class="pg-item ">
        <<?php if ($pageid == $i): ?>span<?php else: ?>a<?php endif ?> class="hy-btn hy-btn-danger <?php if ($pageid == $i): ?>disabled<?php endif ?>" href="<?php HYBBS_URL('my',$data['user'],['post'=>$i]); ?>">
            <?php echo $i;?>
    ...
    <span class="pg-item" >
        <a class="next hy-btn hy-btn-danger" href="<?php HYBBS_URL('my',$data['user'],['post'=>$pageid+1]); ?>"><?php echo $_LANG['下一页'];?></a>
    </span>
    <?php endif ?>
