...
<a class="hy-header-nav hy-header-right icon icon-pencil2" href="<?php if (IS_LOGIN): ?><?php HYBBS_URL('post','index',['fid'=>$fid]) ?><?php else: ?><?php HYBBS_URL('user','login') ?><?php endif ?>" ajax="true" pos="right" rgb="#f1f4f9"></a>
...
<div style="padding-bottom: 5px;position: relative;">
	...
	<span class="Select">
		<select class="Select-input FormControl">
			<option value="<?php HYBBS_URL('forum',$fid,'new'); ?>" <?php if (isset($_GET['HY_URL'][2])): ?><?php if ($_GET['HY_URL'][2] == 'new'): ?>selected<?php endif ?><?php endif ?>><?php echo $_LANG['最新主题'];?></option>
			<option value="<?php HYBBS_URL('forum',$fid,'btime'); ?>" <?php if (isset($_GET['HY_URL'][2])): ?><?php if ($_GET['HY_URL'][2] == 'btime'): ?>selected<?php endif ?><?php endif ?>><?php echo $_LANG['最新回复'];?></option>
  ...
  <div class="rebtn bor" >
	...
	<div class="thread">
		...
		<a class="list-title" href="<?php HYBBS_URL('thread',$v['tid']); ?>" ajax="true" pos="right">
			<h3><?php echo $v['title'];?>
				<?php if ($v['digest']): ?><i class="icon icon-fire" title="<?php echo $_LANG['精'];?>" style="color: tomato !important;;    font-size: inherit;"></i><?php endif ?>
		        	<?php if ($v['state']): ?><i class="icon icon-lock" title="<?php echo $_LANG['禁止回复'];?>" style="color: grey !important;s;    font-size: inherit;"></i><?php endif ?>
		    </h3>
		</a>
  ...
  <div class="hy-box hy-bo-t" style="padding:10px;text-align: center;">
    <a href="<?php if ($pageid==1): ?>javascript:void(0);<?php else: ?><?php HYBBS_URL('forum',$fid,[$type=>$pageid-1]); ?><?php endif ?>" class="hy-btn hy-btn-blu1 l <?php if ($pageid==1): ?>disabled<?php endif ?>"><?php echo $_LANG['上一页'];?></a>
    ...
    <?php $tmp = ($forum[$fid]['threads']%$conf['forumlist']) ?(intval($forum[$fid]['threads']/$conf['forumlist'])+1) : intval($forum[$fid]['threads']/$conf['forumlist']) ; ?>
      <select class="Select-input FormControl">
        <?php for ($i=($pageid-5 < 1) ? 1 : $pageid -5; $i< (($pageid + 5 > $tmp) ? $tmp+1 : $pageid + 5) ; $i++): ?>
          <option value="<?php HYBBS_URL('forum',$fid,[$type=>$i]); ?>" <?php if ($pageid == $i): ?>selected<?php endif ?>><?php echo $_LANG['第'];?> <?php echo $i;?> <?php echo $_LANG['页'];?></option>
    ...
    <a href="<?php if ($pageid==$page_count): ?>javascript:void(0);<?php else: ?><?php HYBBS_URL('forum',$fid,[$type=>$pageid+1]); ?><?php endif ?>" class="hy-btn hy-btn-blu1 r <?php if ($pageid==$page_count): ?>disabled<?php endif ?>"><?php echo $_LANG['下一页'];?></a>
